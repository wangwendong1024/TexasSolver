...
chcp 65001 >nul
echo ========================================
echo Complete CUDA + OpenMP Test Compilation
echo ========================================
echo.
...
echo Compiling complete_cuda_openmp_test.cu...

"C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v%CUDA11_VERSION%\bin\nvcc.exe" ^
    -o build\complete_cuda_openmp_test.exe ^
    complete_cuda_openmp_test.cu ^
    -lcudart -lcublas ^
    -std=c++17 ^
    -O3 ^
    -arch=sm_75 ^
...
echo 1. build\complete_cuda_openmp_test.exe - CUDA+OpenMP test program
echo.

echo Usage:
echo 1. Run test program: build\complete_cuda_openmp_test.exe
